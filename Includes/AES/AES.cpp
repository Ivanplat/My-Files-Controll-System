...
int AES::SetRightStringLengh(std::string& str)
{
    auto slend = str.length() % 16;
    auto d = 16 - slend;
    if (slend != 0)
    {
...
unsigned char* AES::StringToUCharPtr(std::string& str)
{
    const char* fd = str.c_str();
    const char* tempConstCharPtr = reinterpret_cast<const char*>(fd);
    char* tempCharPtr = _strdup(tempConstCharPtr);
    unsigned char* result = (unsigned char*)tempCharPtr;
    return result;
}

void AES::RemoveUnusedChars(std::string& str, int count)
{
